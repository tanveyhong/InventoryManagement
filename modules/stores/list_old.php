...
$all_stores = [];
$all_products = [];
$fetchedFromPostgreSQL = false;

// Try to fetch from PostgreSQL if cache is stale or refresh requested
if ($shouldRefreshCache || !$cacheIsFresh) {
    try {
        $sqlDb = SQLDatabase::getInstance();
        
        // Fetch active stores from PostgreSQL
        $all_stores = $sqlDb->fetchAll("SELECT * FROM stores WHERE active = TRUE ORDER BY name ASC");
        
        // Fetch active products for overview
        $all_products = $sqlDb->fetchAll("SELECT * FROM products WHERE active = TRUE ORDER BY name ASC LIMIT 300");
        
        error_log("PostgreSQL fetch - Stores count: " . count($all_stores) . ", Products count: " . count($all_products));
        
        // Check if we got data
        if (is_array($all_stores) && count($all_stores) > 0) {
            // Save to cache
            $cacheData = [
            ...
            $fetchedFromPostgreSQL = true;
            error_log("PostgreSQL fetch successful - saved to cache");
            ...
            error_log("PostgreSQL returned empty data");
            throw new Exception('PostgreSQL returned no data');
        }
    } catch (Exception $e) {
